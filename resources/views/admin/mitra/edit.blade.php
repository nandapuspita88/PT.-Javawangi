...
<div class="container my-3">
    <h3 class="my-3 text-center">Edit Data Mitra</h3>
    <form action="/admin/mitra/{{ $mitra->id }}" method="post" enctype="multipart/form-data">
        @method('put')
        @csrf
    <div class="row mb-3">
    ...
    <div class="row mb-3">
        ...
        <div class="col-sm-10">
        <input type="text" class="form-control" id="alamat" name="alamat" value="{{ old('alamat',$mitra->alamat) }}">
        </div>
    </div>
    <div class="row mb-3">
        <label for="email" class="col-sm-2 col-form-label">Email</label>
        <div class="col-sm-10">
        <input type="text" class="form-control" id="email" name="email" value="{{ old('email',$mitra->email) }}">
        </div>
    </div>
    <div class="row mb-3">
        <label for="telp" class="col-sm-2 col-form-label">No.Telp</label>
        <div class="col-sm-10">
        <input type="text" class="form-control" id="telp" name="telp"  value="{{ old('telp',$mitra->telp) }}">
        </div>
    </div>
    <div class="row mb-3">
        ...
        <select class="col-sm-10 form-control" aria-label="Default select example" id="jenis" name="jenis">
            <option selected value="{{ old('jenis',$mitra->jenis) }}">{{ old('jenis',$mitra->jenis) }}</option>
